...
# Load database schema for scenario routing decisions
try:
    schema = load_database_schema(
        current_path.parent / "sql_agent", "schema/schema.json"
    )
except FileNotFoundError:
    # If schema doesn't exist, scenario will work without SQL routing capability
    schema = None


class TestScenarioEndToEnd:
    """Scenario End-to-End evaluation tests using FastAPI endpoint."""
    ...
    def teardown_class(self):
        ...
        model_info = get_model_info_for_test("scenario_e2e")
        save_test_report(self.results, "scenario_e2e", model_info)
    ...
    def test_scenario_e2e(self, fixture_name, fixture, test_client, test_notifications):
        """Run Scenario E2E test with optional LLM judge evaluation."""
        ...
        session_id = f"test-scenario-{fixture_name}"
        ...
        response = test_client.get("/scenario", params=params, headers=headers)
